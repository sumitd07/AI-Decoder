...
## 25. CHALLENGE — fragment `related.text` in batches 002/004 (fix the data, not the renderer)
**Symptom.** DB-seeded terms showed broken "Related" rows in the web modal — e.g. "Context length" listed "Measured in", "The window itself is the", "Compared with": lead-in fragments with no visible target term.
**Root cause.** `renderModal()` prints `related[].text` verbatim (arrow, no term name) and `STYLE.md` defines `text` as "how it connects, in a phrase" — self-contained. The original 15, foundations, and batches 001/003/005/006/007 all follow that contract. Batches 002 and 004 (248 entries across 100 concepts) were authored as fragments that assumed the target term's name would be appended. `validate_and_emit.js` validates ids, not prose, so the style break sailed through.
**Decision.** Fix the data, keep the renderer. The renderer is shared with the static bundle (which is correct) and matches the documented contract; teaching it a "does this text look complete?" heuristic would leave wrong data in the DB for every future consumer. Each fragment was rewritten as a complete phrase naming its target ("Measured in" → "Measured in tokens."), auto-appended with casing rules (acronyms/AI/LoRA/Adam keep case, common nouns lowercase) plus ~30 hand-curated rows where blind append failed: sentences the author had split across two rows ("the thing" / "minimizes; broader term is" → "What gradient descent minimizes." / "The broader term is objective function."), "and …" continuation rows ("and dropout." → "Also includes dropout."), and plural/grammar mismatches ("Emits one" + Tokens → "Emits one token at a time.").
**Rejected.** (a) Renderer heuristic (append term name when text lacks terminal punctuation) — 0 false positives on current data, but it papers over bad data and every consumer must re-implement it. (b) Direct REST patch — probed live: reads with the anon key work, but an UPDATE returns 200 with zero rows (RLS silently filters writes), so the SQL-Editor file pattern stands.
**Shipped.** `seed_002.sql`/`seed_004.sql` rewritten in place, `seed_all.sql` regenerated (same concat as the emitter), and a standalone idempotent `supabase/patch_related_fragments.sql` (100 UPDATEs) so the live DB can be fixed without re-running the full seed. Integrity-verified: 248/248 entries parse as JSON, end with terminal punctuation, start uppercase, and keep their original concept→target pairings. **Pending:** Sumit runs `patch_related_fragments.sql` in the SQL Editor; verification after is one anon REST read (`concepts?id=eq.contextlength&select=related`).
**Lesson.** A generator that validates structure but not register lets content-contract breaks straight into the DB. `validate_and_emit.js` now lints `related.text` — empty or missing terminal punctuation (a perfect discriminator on this corpus: 0 false positives across all 9 seed files) fails the batch like a dangling id does. Verified: a fragment-style test batch exits 1 and writes nothing.

## 26. CHALLENGE — extension sign-in never persisted (popup closed mid-flow)
**Symptom.** Clicking "Sign in" in the popup, completing Google OAuth, then reopening the popup showed signed-out — every time.
**Root cause.** `popup.js` called `Supa.signIn()` directly, which runs `chrome.identity.launchWebAuthFlow` inside the *popup's* JS context. Opening the auth window steals focus, and Chrome tears down the action popup the instant that happens — killing the in-flight `await` before `saveSession()` could write the session to `chrome.storage.local`. The OAuth round-trip itself always succeeded; only the write-back was lost.
**Fix.** Moved the OAuth flow into `background.js` (the service worker survives the popup closing). `chrome.runtime.onMessage` gained two message types: `{type:"signin"}` → `self.DecoderSupa.signIn()`, responds `{ok:true,email}`; `{type:"signout"}` → `self.DecoderSupa.signOut()`, responds `{ok:true}` — both routed through the existing catch → `{ok:false,error:String(e)}` shape. `popup.js`'s sign-in/sign-out click handlers now call `chrome.runtime.sendMessage(...)` instead of `Supa.signIn()`/`Supa.signOut()` directly; reads (`Supa.session()`, `Supa.list()`, `Supa.remove()`) are unchanged since they don't need to survive a popup teardown. In the common case the response to `{type:"signin"}` never arrives (popup is already gone) — that's expected; the next popup open reads the session `background.js` saved via `Supa.session()`. Added a one-line comment on `signIn()` in `supa.js` flagging it must be called from the background worker. Version bumped 1.0.1 → 1.0.2.
**Companion requirement (not code).** Supabase Auth → URL Configuration → Redirect URLs must allowlist `https://<extension-id>.chromiumapp.org/*`. Without it, Supabase falls back to the Site URL and signs the *web app* in instead of returning the token to the extension's redirect — a silent failure mode distinct from this bug, worth checking if sign-in still doesn't stick after this fix.
**Verified.** `node --check` on `background.js`, `popup.js`, `supa.js`; message shapes cross-checked by hand between the two files. Real-Chrome pass still pending (sandbox has no browser).
