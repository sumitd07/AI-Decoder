...
A plain-language glossary for the language of AI. Look up any term in seconds, understand it with a real example, see whether it still matters — and save the ones you stumble on.

Decoder is three things:

- **Web app** (`web/`) — a fast lookup tool with a ⌘K search, concept cards, an "explain it plainly / in practice / technically" toggle, and a personal saved terms. Optional Google sign-in syncs your saved terms across devices.
...
**Web app:** open `web/index.html` in a browser. It works immediately in local-only mode (saved terms saved on-device). To turn on Google sign-in and cross-device sync, follow [`SETUP.md`](SETUP.md).
